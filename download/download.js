...
const links = {
  apple: 'https://github.com/PrototypeVersions/ManifestNetwork/releases/download/v0.2.4/Manifest-0.2.4-Apple-Silicon.dmg',
  intel: 'https://github.com/PrototypeVersions/ManifestNetwork/releases/download/v0.2.4/Manifest-0.2.4-Intel.dmg',
  windows: 'https://github.com/PrototypeVersions/ManifestNetwork/releases/download/v0.2.4/Manifest-0.2.4-Windows-x64.exe'
};
...
async function refreshReleaseAvailability() {
  ...
  try {
    ...
    if (pending > 0) {
      if (recommendation) recommendation.textContent = 'The 0.2.4 installers are still being published. Download buttons will activate automatically as each installer becomes available.';
      window.setTimeout(refreshReleaseAvailability, 30000);
    }
  } catch (_) {
    downloadLinks.forEach(markPending);
    if (recommendation) recommendation.textContent = 'The 0.2.4 installers are finishing publication now. Download buttons will activate automatically when the release is ready.';
    window.setTimeout(refreshReleaseAvailability, 30000);
  }
}
